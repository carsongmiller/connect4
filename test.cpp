...
int minimax(int board[][w_], int maximizer, int minormax, int depth)
{
	...
	for (C = 0; C < w_; C++)
	{
		...
		if (R != -1)
		{
			...
			{
				if(minormax == cDisc)
					score[C] = minimax(board, maximizer, pDisc, depth + 1);
				else
					score[C] = minimax(board, maximizer, cDisc, depth + 1);
			}
			...
			score[C] = -1000001; //INT_MIN will be the value for an invalid cell
	}
