...
int main()
{
	...
	cout << "hTrapDetect: " << hTrapDetect(board, pDisc) << endl;
...
void minimax(int board[][w_], long int score[], int who, int currentCheck, int iter, int turn)
{
	...
	if (iter <= MAX_ITER)
	{
		for (int i = 0; i < w_; i++)
		{
			...
			if (rowPlayed != -1)
			{
				/*if (winDetect(newBoard, rowPlayed, i, currentCheck)) //checks computer win
				{
				...
				if(true) //if no wins were found
				{
					if (turn >= 2)
					{
						hTrap = hTrapDetect(board, currentCheck);
							...
							if (hTrap != -1)
							{
								/*if(currentCheck == cDisc)
									...
									score[i] -= pow(1000, MAX_ITER - iter);
							}

						if (turn >= 20)
						{
							vTrap = vTrapDetect(board, currentCheck);

							if (vTrap != -1) //checks for a vertical trap in favor of the computer
							{
								if (currentCheck == cDisc)
									score[vTrap] += pow(3, MAX_ITER - iter);
								else
									score[vTrap] -= pow(3, MAX_ITER - iter);
							}
						}
