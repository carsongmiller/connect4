...
void compTurn(int board[][w_], int &rowPlayed, int &colPlayed, int &turn, int MAX_DEPTH)
{
	...
	colPlayed = ab_minimax(board, cDisc, cDisc, 1, MAX_DEPTH, alpha, beta); //starting alpha and beta out arbitrarily large
	...
	for (int r = 0; r < h_; r++)
	{
		...
		for (int c = 0; c < w_; c++)
		{
			...
			if (board[r][c] == nDisc)
				...
				printColor(getChar(board, r, c), FOREGROUND_RED | FOREGROUND_INTENSITY);
			else if (board[r][c] == cDisc)
				printColor(getChar(board, r, c), FOREGROUND_BLUE | FOREGROUND_INTENSITY);
